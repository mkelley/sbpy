...
"""
====================
sbpy.data.ephem.core
====================

Defines the Ephem class for storing and querying ephemerides.
...
class Ephem(DataClass):
        ...
        * By default, all properties are provided in the J2000.0 reference
        ...
        if epochs is None:
            ...
            if epochs.scale != "utc":
                warn(
                    (
                        "converting {} epochs to utc for use in "
            ...
            if "step" in _epochs:
                if _epochs["step"].unit is not u.dimensionless_unscaled:
                    _epochs["step"] = "{:d}{:s}".format(
                        ...
                        {u.minute: "m", u.hour: "h", u.d: "d", u.year: "y"}[
                            _epochs["step"].unit
                        ],
                    )
                else:
                    _epochs["step"] = "{:d}".format(int(_epochs["step"].value - 1))
        else:
        ...
        for targetid in targetids:
            ...
            obj = Horizons(
                id=targetid, id_type=id_type, location=location, epochs=_epochs
            )
            try:
                ...
                raise QueryError(
                    (
                        ...
                        "The following query was attempted: {:s}"
                    ).format(str(e), obj.uri)
                )
        ...
        all_eph["epoch"] = Time(all_eph["datetime_jd"], format="jd", scale="utc")
        ...
        if "solartime" in all_eph.colnames:
            all_eph["solartime"] = Angle(all_eph["solartime"], "hr").hourangle * u.hr

        if "locapp_hourangle" in all_eph.colnames:
            all_eph["locapp_hourangle"] = u.Quantity(
                Angle(all_eph["locapp_hourangle"], "hr"), u.hourangle
            )
    ...
    @cite(
        {
            "data source": "https://minorplanetcenter.net/iau/MPEph/MPEph.html",
            "software: astroquery": "2019AJ....157...98G",
        }
    )
    def from_mpc(
        ...
        * See `astroquery.mpc.MPC.get_ephemerides` and the Minor
        ...
        if epochs is None:
            ...
            if _epochs.scale != "utc":
                warn(
                    ("converting {} epochs to utc for use in " "astroquery.mpc").format(
                        _epochs.scale
                    ),
                    TimeScaleWarning,
                )
            ...
            if start.scale != "utc":
                warn(
                    (
                        "converting {} start epoch to utc for use in " "astroquery.mpc"
                    ).format(start.scale),
                    TimeScaleWarning,
                )
                start = start.utc
            ...
            if stop is not None and stop.scale != "utc":
                warn(
                    (
                        "converting {} stop epoch to utc for use in " "astroquery.mpc"
                    ).format(stop.scale),
                    TimeScaleWarning,
                )
                stop = stop.utc
        ...
        for targetid in targetids:
            try:
                ...
                if start is None:
                    ...
                    for i in range(len(_epochs)):
                        e = MPC.get_ephemeris(
                            targetid,
                            location=location,
                            start=Time(_epochs[i], scale="utc"),
                            number=1,
                            **kwargs,
                        )
                ...
                raise QueryError("Error raised by astroquery.mpc: {:s}".format(str(e)))

            # add targetname column
            eph.add_column(Column([targetid] * len(eph), name="Targetname"), index=0)
        ...
        * By default, all properties are provided in the J2000.0 reference
        ...
        if epochs is None:
            ...
            if epochs.scale != "utc":
                warn(
                    (
                        "converting {} epochs to utc for use in " "astroquery.imcce"
                    ).format(epochs.scale),
                    TimeScaleWarning,
                )
            ...
            if _epochs["start"].scale != "utc":
                warn(
                    (
                        "converting {} start epoch to utc for use in "
                        "astroquery.imcce"
                    ).format(_epochs["start"].scale),
                    TimeScaleWarning,
                )
            ...
            if "stop" in _epochs and _epochs["stop"].scale != "utc":
                warn(
                    (
                        "converting {} stop epoch to utc for use in " "astroquery.imcce"
                    ).format(_epochs["stop"].scale),
                    TimeScaleWarning,
                )
            ...
            if "number" in _epochs:
                ...
                _epochs["step"] = (
                    int(
                        (Time(_epochs["stop"]) - Time(_epochs["start"])).jd
                        * 86400
                        / (_epochs["number"] - 1)
                    )
                    * u.s
                ...
                _epochs["number"] = (
                    (Time(_epochs["stop"]) - Time(_epochs["start"])).jd
                    * 86400
                    / _epochs["step"].to("s").value
                ) + 1
            if "step" in _epochs:
                _epochs["step"] = "{:f}{:s}".format(
                    _epochs["step"].value,
                    {u.s: "s", u.minute: "m", u.hour: "h", u.day: "d"}[
                        _epochs["step"].unit
                    ],
                )
        else:
        ...
        if isinstance(location, EarthLocation):
            location = "{:+f} {:+f} {:.1f}".format(
                location.lon.deg, location.lat.deg, location.height.to("m").value
            )
    ...
    @classmethod
    @requires("pyoorb")
    @cite({"method": "2009M&PS...44.1853G", "software": "https://github.com/oorb/oorb"})
    def from_oo(
        cls,
        orbit,
        epochs=None,
        ...
        for testtype in ["KEP", "COM", "CART"]:
            field_names = [field[0] for field in Conf.oorb_orbit_fields[testtype]]
            try:
        ...
        for colname in orb.field_names:
            if colname in default_units.keys() and not isinstance(
                orb[colname], (u.Quantity, u.CompositeUnit, Time)
            ):
        ...
        try:
            epochs = list(zip(epochs.mjd, [Conf.oorb_timeScales["TT"]] * len(epochs)))
        except TypeError:
        ...
        if scope == "full":
            ...
            ephem = cls.from_columns(oo_eph_col_u, names=Conf.oorb_ephem_full_fields)
        elif scope == "basic":
            for i, col in enumerate(oo_eph_col):
                oo_eph_col_u.append(
                    Ephem._unit_apply(col, Conf.oorb_ephem_basic_units[i])
                )
            ephem = cls.from_columns(oo_eph_col_u, names=Conf.oorb_ephem_basic_fields)
        ...
        ephem.table["epoch"] = Time(ephem["MJD"], format="mjd", scale=timescale.lower())
